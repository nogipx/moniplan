...
def ask_license_type(update: Update, context: CallbackContext) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton(
            f"Стандартная", callback_data=f'type_{LICENSE_TYPE_STANDARD}')],
        [InlineKeyboardButton(
            f"Премиум", callback_data=f'type_{LICENSE_TYPE_PREMIUM}')],
        [InlineKeyboardButton(
            f"Бизнес", callback_data=f'type_{LICENSE_TYPE_BUSINESS}')],
        [InlineKeyboardButton("Отмена", callback_data='cancel')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def handle_license_type_selection(update: Update, context: CallbackContext) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton(
            "1 месяц", callback_data=f'duration_{LICENSE_DURATION_30}')],
        [InlineKeyboardButton(
            "6 месяцев", callback_data=f'duration_{LICENSE_DURATION_180}')],
        [InlineKeyboardButton(
            "1 год", callback_data=f'duration_{LICENSE_DURATION_365}')],
        [InlineKeyboardButton(
