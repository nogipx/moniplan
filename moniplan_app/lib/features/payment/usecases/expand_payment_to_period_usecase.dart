...
class ExpandPaymentToPeriodUseCase implements IUseCase<ExpandPaymentToPeriodUseCaseResult> {
  ...
  ExpandPaymentToPeriodUseCaseResult run() {
    ...
    if (!payment.isRepeat) {
      final originalPayment = payment.copyWith(plannerId: plannerId);
    ...
    final payments =
        generatedDates.map((e) {
          ...
          if (date.isSameDay(baseDate)) {
            return payment.copyWith(date: date, plannerId: plannerId);
          }
